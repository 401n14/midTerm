...
// Store username and language for each socket connection
let username;
...
socket.on('connect', () => {
  ...
  rl.question(chalk.hex('#FF9F1C')('What is your name? '), answer => {
    ...
    rl.question(
      chalk.hex('#FF9F1C')('What language do you speak? '),
      language => {
...
socket.on('message', data => {
  console.log(chalk.hex('#FF9F1C').bold(`${data.user} (${data.language}): `) + `${data.message}`);
});
